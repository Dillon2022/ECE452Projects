...
def go_to_goal(Ab,TR,cap,goal_id,goal,hit,deg_eps,dist_eps,last_proportional,angle_to_goal):
    ...
    while cap.isOpened():
        ...
        if abs(derivative) >= 1000: #Originally 500 
            state = 1
            print("Hit line...")
            hit.x = p_gc[0]
            hit.z = p_gc[2]
            ...
            if (len(corners)!=0):
                ...
                for i,_ in enumerate(rvecs):
                    ...
                    #       The code is very similar to above where the Goal marker is detected.
                        ...
                        cv2.aruco.drawAxis(frame, mtx, dist, rvecs[i], tvecs[i], 0.05)
                        g_gc = g_gh1.dot(utils.cvdata2transmtx(rvecs[i],tvecs[i])[0])
                        p_gc = g_gc[:,3]
                        cur_dist_helper1 = utils.distance(p_gc[0]-goal.x,p_gc[2]-goal.z)
                        cur_dist_hit = utils.distance(p_gc[0]-hit.x,p_gc[2]-hit.z)
                        if cur_dist_helper1 <= gl_dist:
                            gl_dist = cur_dist_helper1
                            leave.x = p_gh1[0]
                            leave.z = p_gh1[2]
                            count += 1
                        if cur_dist_hit <= dist_eps and count > 100:
                        ...
                        cv2.aruco.drawAxis(frame, mtx, dist, rvecs[i], tvecs[i], 0.05)
                        g_gc = g_gh2.dot(utils.cvdata2transmtx(rvecs[i],tvecs[i])[0])
                        p_gc = g_gc[:,3]
                        cur_dist_helper2 = utils.distance(p_gc[0]-goal.x,p_gc[2]-goal.z)
                        cur_dist_hit = utils.distance(p_gc[0]-hit.x,p_gc[2]-hit.z)
                        if cur_dist_helper2 <= gl_dist:
                            gl_dist = cur_dist_helper2
                            leave.x = p_gc[0]
                            leave.z = p_gc[2]
                            count += 1
                        if cur_dist_hit <= dist_eps and count > 100:
